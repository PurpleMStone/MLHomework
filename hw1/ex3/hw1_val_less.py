...
x = np.empty([12*471, 9], dtype=float)  # training data
    ...
    for day in range(20):
        for hour in range(24):
            if day == 19 and hour > 14:
                continue
            x[month*471+day*24+hour, :] = month_data[month][9, day*24+hour:day*24+hour+9].reshape(1,-1)
...
plt.grid(True)
plt.savefig('loss_lessFeatures.png')
plt.show()

# Save loss data
fileObject = open('train_loss_less.txt', 'w')
for ip in training_loss:
    fileObject.write(str(ip))
    fileObject.write('\n')
fileObject.close()

fileObject = open('val_loss_less.txt', 'w')
for ip in val_loss:
    fileObject.write(str(ip))
    fileObject.write('\n')
fileObject.close()
